...
image: /img/turbo-metrics_blog_11-2024/todo_img.avif
...
The reference implementation for SSIMULACRA2 can be found [here](https://github.com/cloudinary/ssimulacra2). Other popular implementations include [the Rust implementation](https://docs.rs/crate/ssimulacra2_rs/latest), and [Zig implementation](https://github.com/dnjulek/vapoursynth-ssimulacra2).
...
In this experiment, we will evaluate TurboMetrics in terms of performance; that is, the amount of ssimu2 scores computed per unit of time. For that, an episode of an animated series (~25 minutes), encoded with H.264, has been chosen as sample video. However, the kind of video content used, and most of its technical characteristics (e.g. codec, encoding parameters, etc.) except for resolution should be irrelevant, as the same computations are performed for every video of a specific frame dimensions. In this case, the video has a resolution of 1080p (1920x1080 pixels per frame), with 8 bits per pixel, and YUV color space with 4:2:0 chroma subsampling.

The video source and example distorted encode have been compared using **ssimulacra2\_rs** and **TurboMetrics**. I am aware the Zig implementation of SSIMULACRA2 is reportedly faster than ssimulacra2\_rs; however, I am not familiar with Zig or Vapoursynth, so its usage was not considered.

ssimulacra2\_rs has been executed on an Intel i5-11400 CPU (base frequency of 2.60 GHz, turbo frequency of 4.40 GHz), using 6 CPU cores (`-f 6` parameter in ssimulacra2\_rs). TurboMetrics has been executed both on an NVIDIA GTX 1060 GPU, and an NVIDIA RTX 4060 GPU. Both programs have been executed for the same video pair with **varying frame strides: 1, 3, 5, 7, 11, 13, 17, 19 and 23** (not skipping frames, plus every prime number lower than 24 frames / 1 second, excluding 2).

For the CPU and RTX 4060, the distorted video was an AV1 encode of the reference video. For the GTX 1060, as it does not have AV1 decoding hardware support, the H.264 reference video was compared to itself (the computations being performed should be equivalent anyway).

### Results and discussion

The exact results of this experiment are shown in the following table:

| Stride | CPU FPS | GTX 1060 FPS | RTX 4060 FPS |
|:-:|:-:|:-:|:-:|
| 1 | 3.37 | 46.70 | 85.64 |
| 3 | 9.55 | 133.86 | 245.73 |
| 5 | 15.15 | 164.34 | 350.01 |
| 7 | 20.45 | 197.17 | 400.94 |
| 11 | 30.23 | 222.98 | 458.61 |
| 13 | 34.54 | 239.17 | 488.83 |
| 17 | 40.51 | 249.58 | 520.30 |
| 19 | 42.82 | 258.71 | 525.44 |
| 23 | 45.43 | 264.85 | 546.14 |

Here they are in graph form, for better visualization:

![CPU vs GPUs decoded FPS](/img/turbo-metrics_blog_11-2024/ssimu2_cpu-gpu_dec.svg)

On a superficial level, we can already make some observations:
* CPU computation of SSIMULACRA2 scores is quite slow: 3.37 frames per second when using 6 threads (for entire video processing). That's like 0.56 frames per second per thread. I know the CPU used is not a beast by any measure, but beast CPUs are quite expensive, and that is something to consider. This performance still says a lot about the computational complexity of SSIMULACRA2. Those processing speeds are comparable to some of the slower-ish presets of SVT-AV1.
* GPU computation of SSIMULACRA2 scores is impressive, considering the (multithreaded) CPU baseline: Speedups for the GTX 1060 (an 8-year-old GPU) are between x13.86 (full video computation) and x5.83 (stride of 23). Speedups for the RTX 4060 are between x25.41 (full video computation) and x12.02 (stride of 23). It's amazing seeing how GPUs not only achieve real-time SSIMULACRA2 computation of entire videos, but more than x1.94 and x3.56 real-time processing, even (GTX and RTX, respectively).
...
* The GTX 1060 decodes up to 529.7 H.264 (2x264.85) frames per second when using a stride of 23 frames, which corresponds to 75% of its reported peak decoding performance for H.264.
...
![CPU vs GPUs processed FPS](/img/turbo-metrics_blog_11-2024/ssimu2_cpu-gpu_comp.svg)
...
It is worth noting, however, that SSIMULACRA2 is not by any means a kind of "perfect metric" or "absolute truth" about video quality assessment. **SSIMULACRA2 still is unreliable in some situations, and it does not always present a perfect correlation with visual observations**. (For example, while SSIMULACRA2 seems to be great for anime and CGI, some people report it penalizes denoising too harshly, especially compared to VMAF, making it "okay" for other types of content.) In that regard, the results provided by TurboMetrics are not "incorrect", but rather provide a kind of "alternative metric".
 ...
 * The scores provided by the GPU implementation of SSIMULACRA2 are **statistically different** to the real scores (provided by the CPU implementation).
 * **No linear correlation** between the real scores and the GPU scores has been found.
 * TurboMetrics **might tend to under-score** the frames over the real results, although this has yet to be assessed over a larger sample size to prove its veracity.
