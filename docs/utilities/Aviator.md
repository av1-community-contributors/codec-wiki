...
The container your video is stored in is associated with the file extension. Aviator offers two options for video output: the [Matroska](../introduction/terminology.md#mkv--mka--mks--mk3d) video container & the [WebM](../introduction/terminology.md#webm) container. The open-source Matroska container (.MKV) is used by default in Aviator & is a universal multimedia container with broad video & audio support. WebM is designed for web compatibility. Aviator won't copy subtitles to WebM outputs because WebM is only officially compatible with [WebVTT](../subtitles/webvtt.md) subtitles. Both containers work out of the box with Aviator's AV1 video & Opus audio, but WebM output will be disabled if the Copy Audio switch is enabled because then we lose this format compliance assurance.
