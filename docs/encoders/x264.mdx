...
However, when space is not a concern, such as when encoding an intermediate lossless for encoding to a compressed format, x264 can become the fastest lossless codec available, providing super fast encoding and decoding.
For the absolute fastest encoding and decoding, one can use both `--preset ultrafast` and `--tune fastdecode`. However, `--preset superfast` still provides incredibly fast encoding and decoding speed, with 20-30% space savings for lossless,
