...
Currently, there is only one noteworthy community fork of SVT-AV1 called [SVT-AV1-PSY](https://github.com/gianni-rosato/svt-av1-psy).
...
<Tabs>
    <TabItem value="unixlike" label="Linux & macOS">
        *A precompiled AVX2-optimized binary of SVT-AV1-PSY can be installed for x86_64 Linux via [rAV1ator CLI](../utilities/rav1ator-cli.mdx). However, it is always recommended to build from source.*
        ...
        If you'd like to build from the latest release (1.8.0 at the time of writing - last updated 11 Feb 2024) please run `git reset --hard 59645eea34e2815b627b8293aa3af254eddd0d69` in the cloned directory. It is recommended that you do this, as new changes to git aren't always stable right away & a release will guarantee more stability.
...
Sets the minimum flatness of quantization matrices to 0, down from the default 8. This is recommended. The maximum quantization matrix flatness is 15 by default, and should be left alone
