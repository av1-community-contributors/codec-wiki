...
<Tabs>
    ...
    <TabItem value="windows" label="Windows">
        **Clang-CL** is the best option for building in Windows, as it results in faster binaries than building with MSYS2 (either GCC/Clang) and MSVC. The full build process is detailed here.
        ...
        > **Important**: All steps below must be done in **PowerShell**, **not** Command Prompt (cmd.exe).
        ...
        $env:CC = "clang-cl"
        $env:CXX = "clang-cl"
        cmake --fresh -B svt_build -G Ninja -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DSVT_AV1_LTO=OFF -DENABLE_AVX512=OFF -DCMAKE_CXX_FLAGS_RELEASE="-flto /DNDEBUG /clang:-O2 -march=native" -DCMAKE_C_FLAGS_RELEASE="-flto /DNDEBUG /clang:-O2 -march=native"
        ```
